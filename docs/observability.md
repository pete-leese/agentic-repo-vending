...
8. **Zeros with `increase()` / `rate()`** — Propose/vend is a short-lived CLI: each run exports counters once then exits. Prometheus `increase()` ignores the first sample of a new series (treats “appeared with value 1” as unknown baseline), so panels that only used `increase(...[$__range]) or vector(0)` showed **0** even when Explore listed the series. The dashboard queries now use `increase(...) or (metric unless metric offset …)` so the first export counts. **Re-import** the JSON after pulling this change.
9. **Token legend names** — Real SDK usage uses the configured model ids; smoke emits `smoke-model`. `estimate_*` directions mean SDK omitted `RunResult.usage` and we fell back to char-length estimates.
